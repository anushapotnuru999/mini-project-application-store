...
class AppStore extends Component {
  ...
  getFilterApps = searchApps => {
    ...
    const filteredList = searchApps.filter(
      eachOneList => eachOneList.category === activeTabId,
    )
    return filteredList
  }
  ...
  getSearchApps = () => {
    ...
    const searchApps = appsList.filter(eachList =>
  ...
  render() {
    const {activeTabId} = this.state
    const searchApps = this.getSearchApps()
    const filteredList = this.getFilterApps(searchApps)
    ...
    return (
      <div className="app-container">
        ...
        <ul className="list-container">
          {filteredList.map(eachListing => (
            <AppItem eachListing={eachListing} key={eachListing.appId} />
          ))}
        </ul>
      </div>
    )
